	...
	\begin{columns}
		\begin{column}{0.4\linewidth}
			...
			\begin{itemize}
				...
				\only<6->{%
					\item Lots of existing countermeasures~\cite{vy_FederatedLearningBasedIntrusion_2021,yang_Dependablefederatedlearning_2023,zhang_Evaluationdatapoisoning_2022} in \gls{fids} already\dots%
				}
				\only<7>{%
					\item Yet, their impact is difficult to estimate.
					\begin{itemize}
...
\begin{itemize}
    \item Label-flipping can be effective with enough attackers.
    \item Targeted label-flipping strives on well-detected targets.
		\begin{itemize}
			\item but can be significantly mitigated by the model’s generalization capabilities and the use of aggregated classes.
		\end{itemize}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item Build more appropriate metrics for the evaluation of poisoning attacks in \gls{niid} settings.
